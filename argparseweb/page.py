...
class WebuiPage(object):
  ...
  def __init__(self):
    ...
    setattr(self._form, "title", str(self._title))
  ...
  def get_form_inputs(self, parser=None, prefix=[]):
    ...
    if len(mutual_exclusive_list):
      raise UserWarning("Mutual exclusive choices not supported!")
    for group_action in group_actions:
      for action in group_action._group_actions:
        mutual_exclusive_list.append(action)
    # print("dbg!: group_actions", group_actions)
    actions = [action
               for action in parser._actions
               if action not in mutual_exclusive_list]
    # print("dbg!: actions", actions)
    ...
    # mutual excluded probably not supported...

    # for group_action in group_actions:
      
    # test_radio = web.form.Radio('details', ['Home Page', 'Content', 'Contact Us', 'Sitemap'])
    # input_object = test_radio
    # input_object.description = self.get_description(action)
    # input_object.nargs = self.get_nargs(action)
    # input_object.help = self.get_help(action)
    # input_object.disposition = self.get_disposition(action)
